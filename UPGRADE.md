...
An optionnal parameter: ``$driverOptions = array()`` has been added in the following Connection class (in order to make Doctrine\DBAL\Driver\PDOConnection compatible with the interface):
...
If you extends the method ``prepare`` of one of those class, or if you implement the ``Doctrine\DBAL\Connection`` be sure to add the ``$driverOptions = array()`` parameter.
